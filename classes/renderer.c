...
/* {{{ proto Renderer::setClip(Rect rect) */
ZEND_BEGIN_ARG_INFO_EX(php_sdl_renderer_setClip_info, 0, 0, 0)
	ZEND_ARG_OBJ_INFO(0, rect, SDL\\Rect, 1)
ZEND_END_ARG_INFO()

PHP_METHOD(Renderer, setClip)
{
	php_sdl_renderer_t *rt = php_sdl_renderer_fetch(getThis());
	...
	int err = SDL_RenderSetClipRect(rt->renderer, rect);
